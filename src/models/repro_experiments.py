...
if __name__ == "__main__":
    mlflow.start_run(run_name="Quantile Regression Experiment")
    try:
        ...
        mlflow.log_params(
            {
                ...
                "horizons": params.model_params.horizons,
                "quantiles": params.model_params.quantiles,
            }
        )
        ...
        # Log model
        sklearn.log_model(model, "models")
        ...
        # Log metrics
        for idx, (q, h) in enumerate(
            product(params.model_params.quantiles, params.model_params.horizons)
        ):
            loss_value = losses["avg_quantile_loss"][idx]
            mlflow.log_metric(f"loss_q{int(q*100)}_h{h}", loss_value)

        # Log artifacts
        logger.info("Log artifacts")
        mlflow.log_artifact(local_path="dvc.yaml")
        serialize_model(model, "models/model.pkl")
